...
import { Barcode, ChartNoAxesCombined, CircleCheckBig, CreditCard, DollarSign, Store, Truck, XCircle, Zap } from "lucide-react";
...
const paymentMethods = [
  {
    method: "pix",
    icon: <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 30 30" className="dark:invert">
...
export default function PaymentForm({ products, setDeliveryMethod, deliveryMethod }: {
  products: CartItemType[],
  setDeliveryMethod: React.Dispatch<React.SetStateAction<{ method: string, price: number }>>,
  ...
  const sellProducts = async () => {
    ...
    if (paymentMethod === '') return setAlert("Selecione uma forma de pagamento.");
    ...
    const total = products.reduce((total: number, product) => total + (product.price * product.quantity), 0);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4">
        ...
        <ul className="flex gap-4 flex-wrap">
          {deliveryMethods.map((method) => (
            <li key={method.method} onClick={() => setDeliveryMethod({ method: method.method, price: method.price })}>
              <Button
                variant="outline"
      ...
      <div className="flex flex-col gap-4">
        ...
        <ul className="flex gap-4 flex-wrap">
          {paymentMethods.map((method) => (
            <li key={method.method} onClick={() => setPaymentMethod(method.method)}>
              <Button
                variant="outline"
      ...
      <AnimatePresence>
        {show && (
          ...
          >
            <div className="w-full h-full" onClick={() => { setShow(false); router.push("/") }}></div>
            <Card className="absolute aspect-square w-full max-w-[300px] flex flex-col items-center justify-center gap-2 drop-shadow-2xl">
              ...
              <h2 className="font-bold">{message.error !== undefined ? message.error : message.success}</h2>
              <h3 className="text-sm text-muted-foreground">Veja nosso painel de vendas</h3>
